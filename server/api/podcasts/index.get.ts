import { getPodcasts, getPodcastsCount } from '~/server/db/queries'
...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Fetch podcasts (public only)
    const podcasts = await getPodcasts(db, {
      limit,
      offset: (page - 1) * limit,
      status: 'published'
    })
    ...
    // Get total count
    const totalCount = await getPodcastsCount(db, 'published')
    ...
    console.error('[v0] Get podcasts error:', error)
    throw createError({
      statusCode: 500,
      statusMessage: 'Failed to fetch podcasts'
    })
  }
})
